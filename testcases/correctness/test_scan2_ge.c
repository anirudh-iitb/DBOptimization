...
int testval;	
	...
	printf("We test scan by printing all the elements with attribute value greater than or equal to 'b'(the number of printed recIDs should be 667)\n");
	numrec= 0;
	char val = 'b';
	sd = AM_OpenIndexScan(fd,CHAR_TYPE,sizeof(char),GE_OP,&val);
	...
	printf("Testing scan's greater than or equal to operation done\n");
}