...
char testval;	
	...
	printf("Now we delete all the entries for which value=='b'\n");
	testval = 'b';
	sd = AM_OpenIndexScan(fd,CHAR_TYPE,sizeof(char),EQ_OP,(char *)&testval);
	while((recnum=AM_FindNextEntry(sd))>= 0){
		AM_DeleteEntry(fd,CHAR_TYPE,sizeof(char),(char *)&testval,
					recnum);
	}

	printf("Now we print all the elements in the index to check whether deletion has been successful(the number of printed recIDs should be 666)\n");
	numrec= 0;
	sd = AM_OpenIndexScan(fd,CHAR_TYPE,sizeof(char),EQ_OP,NULL);
	while((recnum=AM_FindNextEntry(sd))>= 0){
		printf("%d, ",recnum);
		numrec++;
	}
