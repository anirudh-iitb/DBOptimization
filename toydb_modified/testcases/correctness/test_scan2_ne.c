...
int testval;	
	...
	printf("We test scan by printing all the elements with attribute value not equal to 'b' (the number of printed recIDs should be 666 )\n");
	numrec= 0;
	char val = 'b';
	sd = AM_OpenIndexScan(fd,CHAR_TYPE,sizeof(char),NE_OP,&val);
	...
	printf("Testing scan's not equal to operation done\n");
}