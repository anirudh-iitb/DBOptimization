...
int testval;	
	...
	printf("We test scan by printing all the elements with attribute value equal to 'a'(number of printed values should be 333)\n");
	numrec= 0;
	char val ='a';
	sd = AM_OpenIndexScan(fd,CHAR_TYPE,sizeof(char),EQ_OP,(char*)&val);
	...
	printf("Testing scan's equality operation done\n");
}