...
char testval;	
	...
	printf("We test scan by printing all rec IDs with value > 2200(the number of printed recIDs should be 800)\n");
	int val = 2200;
	int numOfRecords = 0;
	sd = AM_OpenIndexScan(fd,INT_TYPE,sizeof(int),GT_OP,(char *)&val);
	...
	printf("Testing scan's greater than operation done\n");
}