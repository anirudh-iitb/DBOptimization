...

{
	...
	bcopy(&recId,pageBuf+AM_sl+(index-1)*recSize+
	       header->attrLength,AM_si);
...

{
	...
	recSize = header->attrLength + AM_si;
	recIdPtr = PF_PAGE_SIZE;

	for (i = low, j = 1; i <= high; i++,j++)
	{
		offset1 = (i - 1) * recSize + AM_sl;
		offset2 = (j - 1) * recSize + AM_sl;
		bcopy(pageBuf + offset1,tempPage + offset2,header->attrLength + AM_si);
