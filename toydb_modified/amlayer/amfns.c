...

{
	...
	if(status == AM_NOT_FOUND){
		...
		{
			int compare = AM_Compare(pageBuf+AM_sl+(index-1)*recSize+attrLength,attrType,attrLength,&recId);
			if(compare!=0)
			{
				//delete aMAX 
				errVal = PF_UnfixPage(fileDesc,pageNum,FALSE);
	...
	if(deleted0){
		...
		int compare = AM_Compare(pageBuf+AM_sl+(index-2)*recSize+attrLength-AM_si,attrType,AM_si,&zero);
		if(compare!=0) 
