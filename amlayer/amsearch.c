...
int *indexPtr; /* pointer to index in leaf where key is present or can be inserted */
{
	...
	// printf("pagenum of the first page is %d\n",*pageNum);
	...
	{
		// printf("Page number for the probable root is %d\n",*pageNum);
		// printf("It is a root!\n");
		bcopy(*pageBuf,iheader,AM_sint);
		// printf("pagenum on left is %d\n",*(*pageBuf+AM_sint));
		// printf("pagenum on right is %d\n",*(*pageBuf+AM_sint+iheader->attrLength+AM_si));
...

{
	...
	recSize = AM_si  + header->attrLength;
	// if(!with_recID) attrLength = attrLength - AM_si;
	low = 1;
	high = header->numKeys;
	// printf("The total no of keys in the iheader is %d\n",high);
	...
	if ((high - low) == 0)
		...
		else
		{
			...
			bcopy(pageBuf+AM_sint+low*recSize,(char *)&pageNum,
			      AM_si);
			// bcopy(pageBuf+AM_sint+AM_si+attrLength,(char *)&pageNum,
			//       AM_si);
			// printf("wohahahhah pageNum is %d\n",pageNum);
			*indexPtr = low;
			return(pageNum);
		}
...

{
	...
	while ((high - low ) > 1)
	{
		...
		bcopy(pageBuf + AM_sl + (mid - 1)*recSize,&buf,AM_si);
	...
	if ((high - low) == 0)
	{ 
		// bcopy(pageBuf+AM_sl+(low - 1)*recSize, &buf, AM_si);
		// bcopy(value, &val, AM_si);
		// printf("The compared values are %d, %d\n", buf, val);
	...
	if ((high - low) == 1)
	{
		...
		{ 
			bcopy(pageBuf+AM_sl+(low)*recSize, &buf, AM_si);
			bcopy(value, &val, AM_si);
			// printf("The compared values are %d, %d\n", val, buf);
			compareVal = AM_Compare(pageBuf+AM_sl+low*recSize,
			attrType, attrLength, value);
			// printf("CompareVal obtained when %d compared with %d is %d\n",val,buf,compareVal);
			if (compareVal < 0)
			{
				*indexPtr = low + 1;
				return(AM_NOT_FOUND);
...
int attrLength;
{
	...
	case 'i' : 
		{
			...
			if (valint < bufint) retVal = -1;
			else if (valint > bufint) retVal = 1;
			else retVal = 0;
			break;
		}
	case 'f' : 
		{
			...
			if (valfloat < buffloat) retVal = -1;
			else if (valfloat > buffloat) retVal = 1;
			else retVal = 0;
			break;
		}
	case 'c' : 
		{
			// printf("%s compared with %s results in %d\n",valPtr,bufPtr,strncmp(valPtr,bufPtr,attrLength));
			// printf("hsdbhjs\n");
			retVal = strncmp(valPtr,bufPtr,length);
			break;
		}
	}
	...
	if(attrLength!=length)
	{
		if(retVal==0)
		{
			bcopy(bufPtr+length,(char *)&bufint,AM_si);
			bcopy(valPtr+length,(char *)&valint,AM_si);
			// printf("THe values compared are %d, %d\n", valint, bufint);
			if (valint < bufint) retVal = -1;
			else if (valint > bufint) retVal = 1;
			else retVal = 0;
		}
